...
md`# Chess Club Ranking History - Chart by Rank - Simple`
)});

main.variable(observer()).define(["md"], function(md){return(
md`The charts run with a default interpolation value of 10. This value will be changed at Demo time to illustrate how it affects the behaviour of the different charts`
)});

main.variable(observer()).define(["md"], function(md){return(
md`The simple chart works of the default requirements and displays only the Ranking of the Players and their names.`
)});
...
main.variable(observer()).define(["md"], function(md){return(
md`# Chess Club Ranking History - Chart by Rank - Extended With More Attributes From Model`
)});

main.variable(observer()).define(["md"], function(md){return(
md`In addition to the Simple chart, two more attributes have been displayed for each Player - Points and WinRatio.`
)});
main.variable(observer()).define(["md"], function(md){return(
md`The value of the Points attribute determines the width of the Bar. The value of the Points though has no impact on the sequence in which the Bars are drawn - the sorting remains per Rank value in other words as it is in the SImple chart.`
)});
...
main.variable(observer()).define(["md"], function(md){return(
md`Both charts above are bound to the same timeline control because they work of the same dataframes.`
)});
main.variable(observer()).define(["md"], function(md){return(
md`The change of rank position will happen at the same time on both charts even though the bar size will change as the value of Points changes.`
)});
...
main.variable(observer()).define(["md"], function(md){return(
md`In this chart, the value of the Points attribute determines the width of the Bar (as in is in the second Chart), plus now the position of the bard (the sequence in which the bars are drawn) is determined by the value of the Pionts. 
The Rank value is still displayed on the Bar, but has no impact on anything, it becomes just another Attribute, similar to the Name and Winration for instance. 
The ranking on Points opens up a lot of opportunities for making the chart a lot smoother.
At the time of Demo, we can look at different interpolation values and how they impact the behaviour on the different chart types.
If we assume gtaht we are going to use interpotation factor of 10, then the two charts that are sorted by Rank value will change and can only change Bar sequence in every 10 data frames.
The Chart that is sorted by points however has a lot of room for flactuating the values and that will allow for bigger jumps in position changes between Ranking data releases to be interpolated and represented as a gradual shift.
The bar position swithich can happen at any dataframe in other words, not only on every 10 frames as oppose to the first two charts (assuming tah we work with factor of 10 interpolation value)` 
)});                                                                                                                                                                              
...
main.variable(observer("u1")).define("u1", ["chartByRankSimple", "chartByRankExtended","chartByPoints","dataframe"], function(chartByRankSimple,chartByRankExtended,chartByPoints,dataframe){
...
main.variable(observer("u2")).define("u2", ["chartByRankSimple", "chartByRankExtended","chartByPoints","dataframe"], function(chartByRankSimple,chartByRankExtended,chartByPoints,dataframe){
...
main.variable(observer("u3")).define("u3", ["chartByPoints","dataframeByPoints"], function(chartByPoints,dataframeByPoints){
